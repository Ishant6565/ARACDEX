...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const [googleName, setGoogleName] = useState('KAKASHI HATAKE');
  const [googleEmail, setGoogleEmail] = useState('kakashi.hatake@gmail.com');
  ...
  const handleGoogleSignIn = () => {
    ...
    if (!popup || popup.closed || typeof popup.closed === 'undefined') {
      setTimeout(() => {
        const user = loginWithGoogle(googleEmail, googleName, selectedAvatar);
    ...
    // Listen for message from Google Sign-In popup
    const handleAuthMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'ARCADEX_GOOGLE_AUTH_SUCCESS') {
        window.removeEventListener('message', handleAuthMessage);
        const { email, name, avatar } = event.data.user;
        const user = loginWithGoogle(email || googleEmail, name || googleName, avatar || selectedAvatar);
        setIsGoogleLoading(false);
        sound.playSuccess();
        onUserChange(user);
        onClose();
    ...
    // Reset loading if user manually closes the popup without signing in
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-[#0e0e0e] border border-white/[0.08] rounded-[2px] space-y-3">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
                    {AVAILABLE_AVATARS.map(av => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-[2px] border overflow-hidden shrink-0 relative transition-all bg-black ${
                ...
                <button
                  ...
                  className="w-full mt-2 py-3 px-4 bg-white text-black font-sans font-bold text-xs uppercase tracking-wider rounded-[2px] flex items-center justify-center gap-3 hover:bg-cyan-100 transition-all shadow-md active:scale-95"
                >
                  ...
                  <span>{isGoogleLoading ? 'CONNECTING VIA GOOGLE...' : 'CONTINUE WITH GOOGLE'}</span>
                </button>
              </div>
            </div>
          )}
